...
const UserPersonalDetails = ({ addUserForm, open, preData, roleList }) => {
  // usestate
  ...
  // const validateDoe = (doj, doe) => {
  //   if (!doj || !doe) {
  //     return
  //   }
  //   const dojDate = new Date(moment(doj, 'DD/MM/YYYY'))
  //   const doeDate = new Date(moment(doe, 'DD/MM/YYYY'))

  //   if (doeDate < dojDate) {
  //     setError('doe', {
  //       type: 'manual',
  //       message: 'Date of exit should not be less than date of joining'
  //     })
  //   } else {
  //     clearErrors('doe')
  //   }
  ...
  const watchFields = watch(['mobileNo', 'aadhar', 'PAN', 'empCode', 'personalEmail', 'officialEmail'])
      ...
      <Grid container spacing={5}>
        ...
        <Grid item xs={12} md={4}>
          <FormControl variant='standard' fullWidth>
            ...
            />
            {errors.empCode && <FormHelperText sx={{ color: 'error.main' }}>{errors.empCode.message}</FormHelperText>}
          </FormControl>
        </Grid>
        <Grid item xs={12} md={4}>
          <FormControl variant='standard' fullWidth>
                  ...
                  <DatePickerWrapper sx={{ '& .MuiFormControl-root': { width: '100%' } }}>
                    <DatePicker
                      ...
                      onChange={date => {
                        ...
                        // validateDoe(date, getValues('doe'))
                        trigger('doe')
                      }}
                      selected={field.value && new Date(moment(field.value, 'DD/MM/YYYY'))}
                      customInput={
                  ...
                  <DatePickerWrapper sx={{ '& .MuiFormControl-root': { width: '100%' } }}>
                    <DatePicker
                      ...
                      onChange={date => {
                        ...
                        // validateDoe(getValues('doj'), date) // Validate doe against doj
                        // trigger('doe')
                      }}
                      selected={field.value && new Date(moment(field.value, 'DD/MM/YYYY'))}
                      customInput={
