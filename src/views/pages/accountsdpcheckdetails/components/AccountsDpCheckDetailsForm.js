...
const Container1 = () => {
  ...
  useEffect(() => {
    const fetchDepositBankNameOptions = async (segment = 'equity}') => {
  ...
  const fetchBankDetails = async () => {
    ...
    try {
      const response = await axios.post(
        `${awsConfig.BASE_URL}/dpcheque/bank-details`,
        {
          ...
          <Grid item xs={12} style={{ paddingTop: '0px' }}>
            <FormControl component='fieldset' error={Boolean(errors.Depository)}>
              <Box display='flex' alignItems='center'>
                ...
                <Typography
                  sx={{
                    fontSize: FontDetails.selectLabel,
                    fontWeight: '600',
                    color: '#818589',
                    marginRight: '10px',
                    marginLeft: '5px'
                  ...
                  render={({ field }) => (
                    <RadioGroup row {...field} aria-label='Depository' name='Depository'>
                      <FormControlLabel
                        ...
                        componentsProps={{
                          typography: {
                            sx: { fontSize: FontDetails.textfieldInput, fontWeight: '600', color: '#818589' }
                          }
                        }}
                        control={
                          <Radio
